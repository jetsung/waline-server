...
use crate::schema::create_tables_mysql;
...
impl MysqlAdapter {
    pub async fn new(
        ...
        let ssl_param = if ssl { "true" } else { "false" };
        let charset_param = charset.unwrap_or("utf8mb4");
        let database_url = format!(
            "mysql://{user}:{password}@{host}:{port}/{db}?ssl-mode={ssl_param}&charset={charset_param}"
        );
        let pool = MySqlPool::connect(&database_url).await.map_err(|e| {
            WalineError::Config(format!("Failed to connect to MySQL: {e}"))
        })?;
        create_tables_mysql(&pool, prefix).await?;
        Ok(Self {
            pool,
    ...
    fn row_to_comment(row: &sqlx::mysql::MySqlRow) -> Comment {
        use sqlx::Row;
        Comment {
    ...
    fn row_to_counter(row: &sqlx::mysql::MySqlRow) -> Counter {
        use sqlx::Row;
        Counter {
            object_id: row.get::<i64, _>("id").to_string(),
...
impl DatabaseAdapter for MysqlAdapter {
    ...
    async fn add_comment(&self, comment: &NewComment) -> Result<Comment> {
        ...
        let id = sqlx::query_as::<_, (i64,)>(&format!(r#"SELECT LAST_INSERT_ID()"#))
            .fetch_one(&self.pool)
            .await?;
    ...
    async fn update_comment(&self, id: &str, update: &CommentUpdate) -> Result<()> {
        ...
        if let Some(ref status) = update.status {
            sqlx::query(&format!(r#"UPDATE `{table}` SET `status` = ? WHERE `id` = ?"#))
                .bind(status)
                .bind(id)
                .execute(&self.pool)
                .await?;
        ...
        if let Some(like) = update.like {
            sqlx::query(&format!(r#"UPDATE `{table}` SET `like` = ? WHERE `id` = ?"#))
                .bind(like)
                .bind(id)
                .execute(&self.pool)
                .await?;
        ...
        if let Some(ref comment) = update.comment {
            sqlx::query(&format!(r#"UPDATE `{table}` SET `comment` = ? WHERE `id` = ?"#))
                .bind(comment)
                .bind(id)
                .execute(&self.pool)
                .await?;
    ...
    async fn total_comments(&self, query: &CommentQuery) -> Result<i64> {
        ...
        let count: (i64,) = if url.is_empty() {
            ...
            sqlx::query_as(&format!(
                r#"SELECT COUNT(*) FROM `{table}` WHERE `url` = ? AND `status` = ?"#
            ))
            .bind(url)
            .bind(status)
            .fetch_one(&self.pool)
    ...
    async fn upsert_counter(&self, url: &str, increment: i64) -> Result<Counter> {
        ...
        sqlx::query(&format!(
            ...
            ON DUPLICATE KEY UPDATE `time` = `time` + ?, `updatedAt` = NOW()"#
        ))
        .bind(url)
        .bind(increment)
        .bind(increment)
    ...
    async fn update_user(&self, id: &str, update: &UserUpdate) -> Result<()> {
        ...
        if update.display_name.is_some() { sets.push("`display_name` = ?"); }
        if update.email.is_some() { sets.push("`email` = ?"); }
        if update.password.is_some() { sets.push("`password` = ?"); }
        if update.url.is_some() { sets.push("`url` = ?"); }
        if update.avatar.is_some() { sets.push("`avatar` = ?"); }
        if update.label.is_some() { sets.push("`label` = ?"); }
        if update.user_type.is_some() { sets.push("`type` = ?"); }
        if sets.is_empty() { return Ok(()); }

        let sql = format!(r#"UPDATE `{table}` SET {} WHERE `id` = ?"#, sets.join(", "));
        let mut query = sqlx::query(&sql);
        if let Some(ref v) = update.display_name { query = query.bind(v); }
        if let Some(ref v) = update.email { query = query.bind(v); }
        if let Some(ref v) = update.password { query = query.bind(v); }
        if let Some(ref v) = update.url { query = query.bind(v); }
        if let Some(ref v) = update.avatar { query = query.bind(v); }
        if let Some(ref v) = update.label { query = query.bind(v); }
        if let Some(ref v) = update.user_type { query = query.bind(v); }
        query = query.bind(id);
    ...
    async fn export_all(&self) -> Result<serde_json::Value> {
        ...
        let comments: Vec<serde_json::Value> = sqlx::query(&format!(r#"SELECT * FROM `{comment_table}`"#))
            ...
            .await?
            .iter()
            .map(|r| serde_json::to_value(&Self::row_to_comment(r)).unwrap_or_default())
            .collect();
        ...
        let counters: Vec<serde_json::Value> = sqlx::query(&format!(r#"SELECT * FROM `{counter_table}`"#))
            ...
            .await?
            .iter()
            .map(|r| serde_json::to_value(&Self::row_to_counter(r)).unwrap_or_default())
            .collect();
        ...
        let users: Vec<serde_json::Value> = sqlx::query(&format!(r#"SELECT * FROM `{users_table}`"#))
            ...
            .await?
            .iter()
            .map(|r| serde_json::to_value(&Self::row_to_user(r)).unwrap_or_default())
            .collect();

        Ok(serde_json::json!({ "Comment": comments, "Counter": counters, "Users": users }))
    }

    async fn import_data(&self, _data: &serde_json::Value) -> Result<()> {
